...
retrieves the current public graph. A fresh full census requires an OpenAIRE
access token in OPENAIRE_ACCESS_TOKEN; cached historical replay does
not. Set OPENAIRE_CACHE_DIR to an earlier cache only when intentionally
replaying a historical snapshot.
"""
...
MAX_WORKERS = max(1, int(os.environ.get("OPENAIRE_MAX_WORKERS", "6")))
# OpenAIRE documents up to 7,200 authenticated requests/hour. The default
# start rate stays below that ceiling with headroom; parallel workers hide
# network latency but cannot exceed this global gate.
AUTHENTICATED_REQUESTS_PER_SECOND = min(
    1.8,
    max(0.1, float(os.environ.get("OPENAIRE_REQUESTS_PER_SECOND", "1.8"))),
)
FROM_YEAR = 2020
TO_YEAR = 2026
TERMS = [
...
def wait_for_rate_slot() -> None:
    ...
    with RATE_LOCK:
        ...
        NEXT_REQUEST_AT = request_at + (1 / AUTHENTICATED_REQUESTS_PER_SECOND)
...
def get_json(url: str, retries: int = 4) -> dict:
    ...
    if not ACCESS_TOKEN:
        raise RuntimeError(
            "A fresh full-census request requires OPENAIRE_ACCESS_TOKEN. "
            "OpenAIRE documents a 60 requests/hour unauthenticated limit, "
            "while this census performs 2,580 dataset/software link checks "
            "plus discovery and organisation requests. Use the public "
            "snapshot/notebook or bounded live recheck without credentials."
        )
    for attempt in range(retries):
        try:
            ...
            if exc.code in (401, 403):
                raise RuntimeError(
                    "OpenAIRE rejected OPENAIRE_ACCESS_TOKEN. Refresh the access token and retry."
                ) from exc
...
def main() -> None:
    if ACCESS_TOKEN:
        print(
            "Fresh requests use authenticated OpenAIRE access and are globally "
            f"throttled to {AUTHENTICATED_REQUESTS_PER_SECOND:.1f}/second.",
            flush=True,
        )
    else:
        print(
            "No access token: cache-only replay mode; fresh requests are disabled.",
            flush=True,
        )
