...
On 8 August the same bounded contract detected Graph drift. The featured dataset total changed from one to zero, so that relation no longer tests pagination; the two software rows remained on page 0 while page 1 returned none, so the connector defect remains reproducible. All eight discovery totals moved, and Q‑NEKO appeared as one exact project record plus three alias-based research-product search hits. Those product hits are not treated as grant-attributed outputs without a verified relation. The machine-readable result and source are published as `live-recheck.json` and `openaire_live_recheck.py`.
