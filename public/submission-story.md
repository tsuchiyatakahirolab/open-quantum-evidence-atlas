...
Only 17 of 645 records (2.6%) expose the complete project–funding–publication–dataset–software chain. The supporting pattern is a visibility cliff: 392 publications (60.8%) connect to at least one project and funder, 179 (27.8%; Wilson 95% reference band 24.4–31.3) to a dataset, and only 48 (7.4%; reference band 5.7–9.7) to software—an 8.2× project-to-software gap. The title-literal sensitivity check preserves the pattern: 51.7% project/funding, 24.1% dataset, and 8.0% software. Publication-year slices preserve the project-to-software gap in every complete year from 2020 through 2025; 2026 is partial through 5 May and is not interpreted as a trend. The former deterministic 250-record sample is retained as a reproducibility check and yields its original 68 dataset and 22 software connections.
...
We then re-ran that exact DOI through the authenticated Alien/OpenAIRE demo. The MCP route resolved the same record and confirmed QIA-Phase1, but its typed dataset/software tools reported non-zero totals without rows. We reproduced the cause: OpenAIRE's link operation defaults to page 0, which returns the one dataset and two software records, while the MCP validator requires page 1 or higher. The Atlas therefore turns an apparent evidence null into a concrete integration finding, supplies a working direct-API fallback, and publishes a static diagnostic plus minimal reproducer: https://atlas.tsuchiyalab.com/reproducibility/openaire-mcp-crosscheck.md
