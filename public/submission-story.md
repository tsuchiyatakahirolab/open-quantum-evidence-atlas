...
We then re-ran that exact DOI through the authenticated Alien/OpenAIRE demo. The MCP route resolved the same record and confirmed QIA-Phase1, but its typed dataset/software tools reported non-zero totals without rows. We reproduced the cause: OpenAIRE's link operation defaults to page 0, which returns the one dataset and two software records, while the MCP validator requires page 1 or higher. The Atlas therefore turns an apparent evidence null into a concrete integration finding, supplies a working direct-API fallback, and publishes a minimal reproducer plus public integration report for the MCP maintainers: https://github.com/tsuchiyatakahirolab/open-quantum-evidence-atlas/issues/5
