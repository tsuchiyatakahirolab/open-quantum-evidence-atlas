...
The clock then moved. On 8 August, the same bounded probe found one exact Q‑NEKO project record and three alias-based research-product search hits, while all eight discovery totals had changed. The featured dataset relation also moved from one row to zero, whereas the two software rows continued to reproduce the MCP page mismatch. We publish that drift separately rather than rewriting the 29 July census. The product hits are search candidates, not claimed Q‑NEKO outputs without a verified project relation. Q‑NEKO therefore becomes a demonstrated time-indexed policy benchmark, not a zero-output verdict.
