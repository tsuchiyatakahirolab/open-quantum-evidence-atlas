...
**DOI:** https://doi.org/10.5281/zenodo.21913413
...
On 29 July 2026, an uncached bounded recheck passed all six historical integrity checks, and an authenticated fresh full-corpus execution exactly reproduced all 645 corpus IDs, all 645 dataset/software link states and all four connection rates. The 20 August bounded recheck detected Graph drift: all eight discovery totals had moved; Q‑NEKO returned one exact project record; five raw research-product alias hits became three after the Atlas's two indexed Zenodo releases were excluded; and all three remaining records resolved through `relProjectCode=101241875`. The featured dataset relation remained at zero rows, while the two software rows still reproduced the page-0 connector mismatch. The aggregate rates remain the verified 29 July census until another authenticated full run is reviewed.
...
Q‑NEKO is a watchlist case, not the observed corpus. Its 29 July absence was an observation state, not a performance verdict. The 8 August project appearance is the first recorded Graph-entry milestone. On 20 August, the three non-self product records remained verified as explicit OpenAIRE graph links to project code 101241875. That relation supports graph-level attribution only; it does not establish causal production, reuse, access, or output quality.
