...
export default function AuditLab() {
  ...
  async function checkWatchlist(event: FormEvent) {
    ...
    try {
      ...
      let data: WatchlistResult & { error?: string };
      try {
        data = JSON.parse(responseText) as WatchlistResult & { error?: string };
      } catch {
        throw new Error(`Live lookup failed (HTTP ${response.status})`);
      }
    ...
    <main className="audit-page">
      ...
      <section className="live-section">
        <div className="audit-shell live-grid">
          <div className="live-copy"><p className="audit-kicker acid">05 · LIVE OBSERVABILITY</p><h2>Check a project<br />before drawing conclusions.</h2><p>Alias lookup queries OpenAIRE through an hourly cached preflight. Search hits are candidates, not grant attribution, and this is not a substitute for the full snapshot audit.</p></div>
          ...
          <div className="watch-results" aria-live="polite">
            ...
            </> : <div className="watch-placeholder"><span>LIVE</span><p>Run the Q‑NEKO aliases to inspect the latest observation state against OpenAIRE.</p></div>}
          </div>
        </div>
      </section>
